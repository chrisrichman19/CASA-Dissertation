...
```{r}
out %>%
  filter(variable %in% colnames(tenure2021)) %>%
  write.csv("tenure.csv", row.names = F)
```


```{r}
out %>%
  filter(variable %in% colnames(hholdcomposition2021)) %>%
  write.csv("hholdcomposition.csv", row.names = F)
```


```{r}
out %>%
  filter(variable %in% colnames(accomtype2021)) %>%
  write.csv("accomtype.csv", row.names = F)
```
...
map <- tm_shape(st_as_sf(households_sf)) +
   tm_fill(
    ...
    palette = "reds",
    alpha   = 0.5,
    title   = "Households owned with a mortage or loan (%)"
  ) +
...
map <- tm_shape(st_as_sf(households_sf)) +
   tm_fill(
    ...
    palette = "reds",
    alpha   = 0.5,
    title   = "Married/Civil partnership with no children (%)"
  ) +
  tm_borders(lwd = 0.5, col = "grey50") +
  tm_shape(target_sf) +
  tm_borders(lwd = 3, col = "black") +
