...
```{r}
accomtype2021 = read_csv("censusdata/accomtype2021.csv") %>%
  clean_names() %>%
...
#2011
accomtype2011 = read_csv("censusdata/accomtype2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,across(
    .cols = where(is.numeric) & !all_of("all_households"),
    .fns = ~ (.x / all_households) * 100,
...
```{r}
#2021
hholdcomposition2021 = read_csv("censusdata/hholdcomposition2021.csv") %>%
  clean_names() %>%
  select(-18) %>%
  transmute(lsoa_code,
            across(
...
#2011
hholdcomposition2011 = read_csv("censusdata/hholdcomposition2011.csv") %>%
  clean_names() %>%
  select(-18) %>%
  transmute(lsoa_code,
            across(
...
```{r}
#2021
tenure2021 = read_csv("censusdata/tenure2021.csv") %>%
  clean_names() %>%
...
#2011
tenure2011 = read_csv("censusdata/tenure2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code, all_households,
            across(
    .cols = where(is.numeric) & !all_of("all_households"),
