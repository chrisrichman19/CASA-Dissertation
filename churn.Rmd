...
```{r}
churn <- read_csv("hh_churn_lsoa11_2023.csv")
```

#### joining by London boundaries

```{r}
london <- st_read("barking/LSOA_2011_London_gen_MHW.shp") %>%
  st_transform(.,27700) 
...
```{r}
RMD_in <- read_csv("rmd/RMD_in_LSOA_releasedata.csv", na= c("*", "NA")) %>%
  clean_names()
```

```{r}
RMD_out <- read_csv("rmd/RMD_out_LSOA_releasedata.csv", na= c("*", "NA")) %>%
  clean_names()
```
