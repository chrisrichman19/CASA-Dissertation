...
```{r}
age2021 <- read_csv("data/age2021.csv") %>%
  clean_names() %>%
...
```{r}
age2011 <- read_csv("data/age2011.csv") %>%
  clean_names() %>%
  mutate(lsoa_code, all_usual_residents,
         aged_0_to_14 = aged_4_and_under + aged_5_to_9 + 
      aged_10_to_14,
...
```{r}
ecoactivity2021 = read_csv("data/ecoactivity2021.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
ecoactivity2011 = read_csv("data/ecoactivity2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
```{r}
NSSEC2021 = read_csv("data/NSSEC2021.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
NSSEC2011 = read_csv("data/NSSEC2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
    .cols = where(is.numeric) & !all_of("all_usual_residents_aged_16_74"),
...
```{r}
occupation2021 = read_csv("data/occupation2021.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
occupation2011 = read_csv("data/occupation2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
```{r}
qualification2021 = read_csv("data/qualification2021.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
...
qualification2011 = read_csv("data/qualification2011.csv") %>%
  clean_names() %>%
  transmute(lsoa_code,
            across(
    .cols = where(is.numeric) & !all_of("usual_residents_aged_16"),
...
out %>%
  ...
  write.csv("agecomp.csv", row.names = F)
```

```{r}
out %>%
  filter(str_detect(variable,"^economically")) %>%
  write.csv("ecocomp.csv", row.names = F)
```


```{r}
out %>%
  filter(variable %in% colnames(qualification2021)) %>%
  write.csv("qualification.csv", row.names = F)
```
...
out %>%
  filter(variable %in% colnames(occupation2021)) %>%
  write.csv("occupation.csv", row.names = F)
